...
void DataBase_Train::newTrain( const QString &input )
{
	int total = 0;
	QString traId;
	int setnr, stanr;
	...
	QStringList qslist,linelist;
	linelist = input.split('\n');
	bool ok;
	QString line, now;
	int l=0;
	//for ( size_t l=0; l<linelist.size(); ++l )
	while( l < linelist.size() )
	{

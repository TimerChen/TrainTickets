...
class vector {
	...
	void resize() {
		if (sz == upbound) {
			...
			T* tmp = reinterpret_cast<T*> (operator new (sizeof(T)*up));
...
public:
	...
	private:
		...
		T **  oriplace;
		size_t pos;
		iterator(T* _it, size_t _pos, T * _c) :it(_it), pos(_pos), oriplace(&_c) {}
	...
	public:
		...
		iterator operator+(const int &n) const {
			//TODO
			return iterator(it + n, pos + n, *oriplace);
		}
		iterator operator-(const int &n) const {
			//TODO
			return iterator(it-n, pos-n,*oriplace);
	...
	private:
		...
		*/
		...
		T ** oriplace;
		size_t pos;
		const_iterator(T* _it, size_t _pos, T* _c) :it(_it), pos(_pos),oriplace(&_c) {}
	public:
		/**
		* return a new iterator which pointer n-next elements
		*   even if there are not enough elements, just return the answer.
		...
		*/
		...
		const_iterator operator+(const int &n) const {
			//TODO
			return const_iterator(it + n, pos + n, *oriplace);
		}
		const_iterator operator-(const int &n) const {
			//TODO
			return const_iterator(it - n, pos - n,*oriplace);
		...
		*/
		const_iterator operator++(int) {
			const_iterator tmp(it, pos,*oriplace);
			it += 1;
			++pos;
			return tmp;
		}
	 ...
	 */
	vector() : sz(0), upbound(0),container(NULL){}
	 ...
	 */
	iterator begin() {
		return iterator(&container[0], 0, container);
	}
	const_iterator cbegin() const {
		return const_iterator(&container[0], 0, container);
	}
	/**
	 * returns an iterator to the end.
	 */
	iterator end() {
		return iterator(&container[sz - 1] + 1, sz,container);
	}
	const_iterator cend() const {
		return const_iterator(&container[sz - 1] + 1, sz,container);
	}
	/**
	...
	iterator insert(iterator pos, const T &value) {
		...
		++sz;
		return iterator(&container[pos.pos], pos.pos, container);
	}
	/**
	...
	iterator insert(const size_t &ind, const T &value) {
		...
		return insert(iterator(&container[ind], ind, container), value);
	}
	/**
	...
	iterator erase(const size_t &ind) {
		...
		return erase(iterator(&container[ind], ind, container));
	}
	/**
